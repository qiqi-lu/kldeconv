...
fig_path_data = [os.path.join(path_specimen, "642"), os.path.join(path_specimen, "560")]
dataset_path = [
...
print(
    "Num of channel: {}, Num of time point: {}, num of methods: {}".format(
        Nc, Nt, Nmeth
    )
)
...
for method in methods:
    print(method)
    video_xy = cv2.VideoWriter(
        os.path.join(path_specimen, f"image_restored_{method}_xy_su.mp4"),
        fourcc,
        10,
        (Nx, Ny),
    )
    video_xz = cv2.VideoWriter(
        os.path.join(path_specimen, f"image_restored_{method}_xz_su.mp4"),
        fourcc,
        10,
        (Nx, Nz_inter),
    )
    ...
    for i in id_timepoint:
        ...
        for id_channel in range(len(fig_path_data)):
            fig_path_sample = os.path.join(fig_path_data[id_channel], f"sample_{i}")

            print("load", fig_path_sample)
            if method == "raw":
                img.append(
                    io.imread(os.path.join(dataset_path[id_channel], "raw", raw_txt[i]))
                )
                ...
                img.append(
                    io.imread(os.path.join(fig_path_sample, method, "deconv.tif"))
                )
        ...
        if method == "raw":
            vmin = [0, 0]
            vmax = [np.percentile(img[..., 0], 99.99), np.percentile(img[..., 1], 99.9)]
        else:
            vmin = [0, 0]
            vmax = [
