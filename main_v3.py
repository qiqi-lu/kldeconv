...
###################################################################################
def rescale(tensor, scale=1.0):
    return torch.clamp(tensor, min=0.0, max=1.0) * scale
...
if model_name == "forw_proj":
    only_net, output_mode, n_blocks, n_features, pixel_binning_mode = (
        True,
        "last-one",
        1,
        2,
    ...
    model_suffix = "_{}_{}_bin_{}_newdata".format(
        n_blocks, n_features, pixel_binning_mode
    )
    lambda_loss = 0.0
...
if model_name == "rlsr":
    if data_set_name == "lung3_synth":
        backbone_type, n_blocks, n_features, kernel_size, bp_ks = (
            "rrdb",
            (1, 5),
            (4, 8),
            3,
    ...
    upsample_mode, RL_version, pixel_binning_mode = (
        "conv_up",
        "RL",
        "ave",
    )  # 'conv_up', 'up_conv', 'RL', 'ISRA'
    constraint_01, batchnorm, cat_x = True, True, False
    if only_net == False:
        fp_mode, fpm, bpm = "pre-trained", 1, 1  # 'known', 'pre-trained', None
    if only_net == True:
        fp_mode, fpm, bpm = None, 1, 1  # 'known', 'pre-trained', None
    (
        use_prior,
        train_only_prior,
        prior_type,
    ...
    if data_set_name in ["tinymicro_synth", "lung3_synth", "biosr_real", "msi_synth"]:
        init_mode = "ave_bicubic"  # 'bicubic', 'ave_bicubic', 'nearest'
    if data_set_name in ["tinymicro_real"]:
        init_mode = "net"  # 'net', 'pre-trained'

    if only_net == True:
        output_mode = "last-one"
        model_suffix = "_block_({},{})_feature_({},{})_{}_only_net_pri_{}".format(
            n_blocks[0],
            n_blocks[1],
            n_features[0],
            n_features[1],
    ...
    if only_net == False:
        ...
        model_suffix = "_iter_{}_block_({},{})_fea_({},{})_bb_{}_init_{}_model_{}_bin_{}_fp_{}_fpm_{}_bn_pri_{}_{}_bpm_{}_lam_{}_24".format(
            n_iter,
            n_blocks[0],
            n_blocks[1],
            n_features[0],
...
if data_set_name in ["tinymicro_synth", "tinymicro_real"]:
    epochs = 3
if data_set_name in ["lung3_synth"]:
    epochs = 62  # 36, 40, 55
if data_set_name in ["biosr_real"]:
    epochs = 24  # 15, 21
# ---------------------------------------------------------------------------------
...
if use_lr_schedule == True:
    ...
    print(
        "Start learning rate: {}, decay every: {}, decay rate: {}".format(
    ...
    data_transform_back = transforms.Compose(
        [
            transforms.Normalize(
                mean=-mean_normlize / std_normlize, std=1.0 / std_normlize
            ),
        ]
    )
...
if data_set_name == "tinymicro_synth":  # TinyMicro (synth)
    ...
    lr_root_path = os.path.join(
        "data", "TinyMicro", "data_synth", "train", "sf_4_k_2.0_gaussian_mix_ave"
    )
...
if data_set_name == "biosr_real":
    ...
    hr_root_path = os.path.join(
        root_path, "BioSR", "data_transform", name_specimen, "train", "GT"
    )
    lr_root_path = os.path.join(
        root_path, "BioSR", "data_transform", name_specimen, "train", "WF"
    )

    hr_txt_file_path = os.path.join(
        root_path, "BioSR", "data_transform", name_specimen, "train.txt"
    )
...
if data_set_name == "lung3_synth":  # Lung3 (synth)
    ...
    lr_root_path = os.path.join(
        ...
        "data_synth",
        "train",
        "sf_4_k_2.0_n_gaussian_std_mix_bin_ave",
    )
...
if model_name == "srcnn":
    model = srcnn.SRCNN(
        in_channels=in_channels, out_channels=in_channels, scale_factor=scale_factor
    ).to(device)
# ---------------------------------------------------------------------------------
if model_name == "srresnet":
    model = srresnet.SRResNet(num_channels=in_channels, scale_factor=scale_factor).to(
        device
    )
# ---------------------------------------------------------------------------------
if model_name == "rrdbnet":
...
# ---------------------------------------------------------------------------------
if model_name == "ddn":
    model = ddn.DenseDeconNet(in_channels=in_channels, scale_factor=scale_factor).to(
        device
    )
# ---------------------------------------------------------------------------------
if model_name == "rcan":
...
if model_name == "rlsr":
    ...
    if fp_mode == "pre-trained":
        if data_set_name == "lung3_synth":
            ...
            model_path = os.path.join(
                "checkpoints",
                data_set_name,
                "forw_proj_bs_16_lr_0.001_1_4_bin_ave",
                "epoch_16272.pt",
            )
        if data_set_name == "biosr_real":
            model_path = os.path.join(
                "checkpoints",
                data_set_name,
                "forw_proj_bs_4_lr_0.001_2_4_bin_ave",
                "epoch_95000.pt",
            )
        model_para = torch.load(model_path, map_location=device)
        backbone = rrdbnet.backbone(
            in_channels=in_channels,
            n_features=n_features[0],
            n_blocks=n_blocks[0],
            growth_channels=n_features[0] // 2,
            bias=True,
        )
        forw_proj = rlsr.ForwardProject(
            backbone=backbone,
            in_channels=in_channels,
            n_features=n_features[0],
            scale_factor=scale_factor,
    ...
    if fp_mode == "known":
        ...
        conv = lambda x: torch.nn.functional.conv2d(
            input=padd(x), weight=ker, stride=1, groups=in_channels
        )
        forw_proj = lambda x: torch.nn.functional.avg_pool2d(
            x, kernel_size=scale_factor, stride=scale_factor
        )
    ...
    model = rlsr.RLSR(
        ...
        pixel_binning_mode=pixel_binning_mode,
        constraint_01=constraint_01,
        forw_proj=forw_proj,
        bn=batchnorm,
        cat_x=cat_x,
        fpm=fpm,
        ...
        train_only_prior=train_only_prior,
        lambda_prior=lambda_prior,
        prior_inchannels=prior_inchannels,
        prior_bn=prior_bn,
    ).to(device)
    ...
    if train_only_prior == True:
        ...
        model_path = os.path.join(
            "checkpoints",
            data_set_name,  # 'rlsr_bs_4_lr_0.005_iter_1_block_(1,5)_fea_8_bb_rrdb_init_ave_bicubic_model_RL_bin_ave_fp_pre-trained_fpm_1_bn_pri_none_1_bpm_1',\
...
for i_epoch in range(epochs):
    ...
    for i_batch, sample in enumerate(train_dataloader):
        ...
        if use_lr_schedule == False:
            ...
            if i_iter >= warm_up:
                for g in optimizer.param_groups:
                    g["lr"] = scheduler_cus["lr"]  # set learning rate

        # ------------------------------------------------------------------------------
        # calculate metrics
        if data_transform_back is not None:
            pred, gt, inpt = (
                data_transform_back(pred),
                data_transform_back(gt),
                data_transform_back(inpt),
            )

        if output_mode in ["each-iter-train", "each-iter-train-prior"]:
            out = pred[-1]
        if output_mode == "last-one":
            out = pred
        # ------------------------------------------------------------------------------
        # plot loss and metrics
        if i_iter % plot_every_iter == 0:
            ave_ssim, ave_psnr = eva.measure(
                img_test=rescale(out, scale=1.0),
                img_true=rescale(gt, scale=1.0),
                data_range=1.0,
            )
            if writer != None:
                ...
                if only_net == False:
                    writer.add_scalar("loss1", loss1, i_iter)
                if model_name == "forw_proj":
                    writer.add_scalar("loss1", loss, i_iter)
        ...
        if i_iter % 200 == 0:
            print(
                "\nEpoch: {}, Iterations: {}, loss_main: {:>.5f}, loss_aux: {:>.5f}, PSNR: {:>.5f}, SSIM: {:>.5f}".format(
                    i_epoch,
                    i_iter,
                    np.mean(print_loss),
                    np.mean(print_loss_aux),
                    np.mean(print_psnr),
                    np.mean(print_ssim),
                )
            )
        ...
        if (i_iter + 1) % save_every_iter == 0:
            ...
            torch.save(
                model_dict, os.path.join(model_path, "epoch_{}.pt".format(i_iter + 1))
            )
        # ----------------------------------------------------------------------------------
        # validation
        if (i_iter % val_every_iter == 0) and (validation_disable == False):
            ...
            for i_batch_val, sample_val in enumerate(valid_dataloader):
                ...
                if output_mode == "last-one":
                    out_val = pred_val
                ave_ssim, ave_psnr = eva.measure(
                    img_test=rescale(out_val, scale=1.0),
                    img_true=rescale(y_val, scale=1.0),
                    data_range=1.0,
                )
            ...
            print(
                "\nValidation, Loss: {:>.5f}, Loss (main): {:>.5f}, PSNR: {:>.5f}, SSIM: {:>.5f}".format(
                    ...
                    running_val_loss_main / num_batches_val,
            ...
            if writer != None:
                writer.add_scalar(
                    "total_loss_val", running_val_loss / num_batches_val, i_iter
                )
                writer.add_scalar(
                    "loss_main_val", running_val_loss_main / num_batches_val, i_iter
                )
                writer.add_scalar(
