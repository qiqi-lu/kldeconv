...
def SSIM(x, y, c1=1e-4, c2=9e-4):
    ...
    - c1 (float, optional): constant for numerical stability. Default: 1e-4.
    - c2 (float, optional): constant for numerical stability. Default: 9e-4.
    ### Returns:
    - ssim (tensor): structural similarity index map.
    """
    if len(y.shape) == 4:
        dims = (-2, -1)  # 2D data
    if len(y.shape) == 5:
    ...
    sigma_cross = torch.mean(
        torch.mul(torch.sub(x, mean_x), torch.sub(y, mean_y)), dim=dims, keepdim=True
    )
