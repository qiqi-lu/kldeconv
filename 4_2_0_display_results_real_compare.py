...
# ------------------------------------------------------------------------------
figure_name, show_image, show_statistic = "2d_real", True, True
# figure_name, show_image, show_statistic = "3d_real", True, True
# figure_name, show_image, show_statistic = "2d_real_many", False, True
num_samples = 20

# ------------------------------------------------------------------------------
# dataset_name, dataset_id, id sample, pos, size
dataset_show_info_list = {
    "2d_real": (
        # ('F-actin',"F-actin-nonlinear-9", 5, (200, 200), 100, (0, 255, 0)),
        # ("CCP", "CCPs-9", 0, (200, 100), 100, (0, 255, 0)),
        ("MT", "Microtubules2-9", 0, (200, 100), 100, (0, 255, 0)),
        ("ER", "ER-6", 0, (200, 100), 100, (0, 255, 0)),
        ("F-actin", "F-actin-9", 0, (200, 100), 100, (0, 255, 0)),
    ),
    "3d_real": (
        (
            "MT-3D",
...
for i_dataset in range(num_datasets):
    dataset_titles.append(dataset_show_info[i_dataset][0])

print("-" * 80)
print(f"[INFO] methods titles: {methods_names}")

# ------------------------------------------------------------------------------
# load results from different methods nad datasets
# ------------------------------------------------------------------------------
results_all = []
for i_dataset in range(num_datasets):
    # load results
    dataset_name_test = dataset_show_info[i_dataset][1]
...
if show_image:
    ...
    for i_dataset in range(num_datasets):
        if ndim == 2:
            ...
            dataset_title, dataset_name_test, id_sample, pos, size, color_img = (
        ...
        for i_meth in range(len(results)):
            ...
            if i_meth == 0:
                axes_tag.text(
                    pos_text[1], pos_text[0], dataset_title, **dict_text_struc
                )
...
if show_statistic:
    ...
    # use the min and max of metrics_dataset as limit
    y_lim_metrics = (
        (metrics_dataset[:, :, :, 0].min(), metrics_dataset[:, :, :, 0].max()),
        (metrics_dataset[:, :, :, 1].min(), metrics_dataset[:, :, :, 1].max()),
        (metrics_dataset[:, :, :, 2].min(), metrics_dataset[:, :, :, 2].max()),
    )
    # adjust limits
    limit_ranges = [lim[1] - lim[0] for lim in y_lim_metrics]
    y_lim_metrics = [
        (lim[0] - 0.05 * limit_ranges[i], lim[1] + 0.05 * limit_ranges[i])
        for i, lim in enumerate(y_lim_metrics)
    ]
    ...
    font_size = 14
    width = 1 / (num_methods + 2)  # the width of the bars
    width_bar = width * 0.8
    ...
    fig, axes = plt.subplots(
        nrows=nr, ncols=nc, figsize=(3 * nc / 2 * num_datasets, 3 * nr), **dict_fig
    )

    # add tickes
    ticks = np.round(np.linspace(0, 60, 25), decimals=1)
    axes[0].set_yticks(ticks)
    axes[0].set_yticklabels(ticks, fontsize=font_size)
    ticks = np.round(np.linspace(0.0, 1.0, 41), decimals=2)
    ...
    for i_metric in range(num_metrics):
        ...
        for i_meth in range(num_methods + 1):
            ...
            ax.boxplot(
                metrics_dataset[:, :, i_meth, i_metric].transpose(),
                positions=x + offset,
                widths=width_bar,
                showfliers=False,
                patch_artist=True,
                boxprops=dict(
                    facecolor=method_colors[i_meth], edgecolor="black", linewidth=1.0
                ),
        ...
        ax.set_xticks(x + width * 2, dataset_titles, fontsize=font_size)
        ax.set_ylim(y_lim_metrics[i_metric])
        ax.set_xlim(-width, num_datasets - width)
        # del right and top axis
        ax.spines["right"].set_visible(False)
        ax.spines["top"].set_visible(False)
        axes[i_metric].set_box_aspect(3 / 2 / num_datasets)
        ...
        for i_dataset in range(num_datasets):
            ...
            for i_pair in range(len(test_pairs)):
                ...
                # use the mean+-std as star_y ----------------------------------
                # star_y = (metrics_mean_value + metrics_std_value)[
                #     i_dataset, test_pairs[i_pair][0]
                # ].max()
                # star_y += (
                # y_lim_metrics[i_metric][1] - y_lim_metrics[i_metric][0]
                # ) * 0.02

                # use the Q3+1.5IQR as star_y ----------------------------------
                data_tmp = metrics_dataset[
                    i_dataset, :, test_pairs[i_pair][0], i_metric
                ]
                Q1, Q3 = np.percentile(data_tmp, 25), np.percentile(data_tmp, 75)
                star_y = Q3 + (Q3 - Q1) * 1.5
    ...
    plt.savefig(
        os.path.join(path_fig, f"image_restored_compare_{figure_name}_metrics.png")
    )
    plt.savefig(
        os.path.join(path_fig, f"image_restored_compare_{figure_name}_metrics.svg")
    )
