...
path_fig_data = [os.path.join(path_specimen, "642"), os.path.join(path_specimen, "560")]
...
for id_channel in range(len(path_fig_data)):
    ...
    for i in id_sample:
        ...
        imgs.append(
            io.imread(
                os.path.join(path_dataset[id_channel], methods[0], raw_txt[i])
            ).astype(np.uint16)
        )
        ...
        #     'y_pred_all.tif'))[-1])
        imgs.append(
            io.imread(os.path.join(path_fig_sample, methods[-1], "y_pred_all.tif"))
        )
...
print(
    "Num of channel: {}, Num of time point: {}, num of methods: {},\
    image shape: {}".format(
        Nc, Nt, Nmeth, (Nz, Ny, Nx)
    )
)
...
for imeth in range(Nmeth):
    ...
    for it in range(Nt):
        ...
        axes[0, it].text(
            10, 20, "{:>.1f} min".format(interval * id_sample[it] / 60), color="white"
        )
        axes[1, it].imshow(utils_plot.render(xz_plane, **dict_img))

    plt.savefig(os.path.join(path_specimen, f"image_restored_{methods[imeth]}.png"))
    plt.rcParams["svg.fonttype"] = "none"
    plt.savefig(os.path.join(path_specimen, f"image_restored_{methods[imeth]}.svg"))
