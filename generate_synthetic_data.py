...
if lamb == None:
    path_dataset_blur = os.path.join(
        ...
        "raw_psf_{}_gauss_{}_poiss_{}_sf_{}_ratio_{}".format(
            s_crop, std_gauss, poisson, scale_factor, ratio
        ),
    )
else:
    path_dataset_blur = os.path.join(
        path_dataset,
        "raw_psf_{}_gauss_{}_poiss_{}_sf_{}_ratio_{}_lambda_{}".format(
            s_crop, std_gauss, poisson, scale_factor, ratio, lamb
        ),
    )
...
for i in id_data:
    ...
    data_blur = dcv.Convolution(data_gt, PSF_crop, padding_mode="reflect", domain="fft")

    # add noise
    data_blur_n = utils_data.add_mix_noise(
        data_blur, poisson=poisson, sigma_gauss=std_gauss, scale_factor=scale_factor
    )
