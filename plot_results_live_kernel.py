...
def plot_profile(axes, psf, bp_kernel, s=None, color=None, label=None):
    ...
    axes[0, 0].plot(
        bp_kernel[S_ker[0] // 2, S_ker[1] // 2, :], color=color, label=label
    )
    axes[0, 1].plot(
        a[S_fft[0] // 2, S_fft[1] // 2, S_fft[2] // 2 :], color=color, label=label
    )
    axes[0, 2].plot(
        b[S_fft[0] // 2, S_fft[1] // 2, S_fft[2] // 2 :], color=color, label=label
    )

    axes[1, 0].plot(
        bp_kernel[:, S_ker[1] // 2, S_ker[2] // 2], color=color, label=label
    )
    axes[1, 1].plot(
        a[S_fft[0] // 2 :, S_fft[1] // 2, S_fft[2] // 2], color=color, label=label
    )
...
axes[0, 0].set_title("BP (x)"), axes[0, 1].set_title("|FT(BP)| (x)"), axes[
    0, 2
].set_title("|FT(FP) x FT(BP)| (x)")
axes[1, 0].set_title("BP (z)"), axes[1, 1].set_title("|FT(BP)| (z)"), axes[
    1, 2
].set_title("|FT(FP) x FT(BP)| (z)")
...
plot_profile(
    axes,
    psf=kernel_fp,
    bp_kernel=kernel_bp[4],
    s=y.shape,
    color=methods_color[0],
    label=methods[4],
)
