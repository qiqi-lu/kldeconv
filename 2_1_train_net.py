...
print("-" * 80)
print(f"[INFO] Number of datasets: {len(datasets_id)}")
...
for dataset_id in datasets_id:
    ...
    print(f"[INFO] Image shape: (input | gt) {img_lr_shape}|{img_hr_shape}")
    ...
    num_epochs = (
        params["num_iter_train"] // num_batches_train
        if params["num_iter_train"] % num_batches_train == 0
        else params["num_iter_train"] // num_batches_train + 1
    )

    try:
        pbar_epoch = tqdm.tqdm(
            total=num_epochs, desc="[INFO] Epoch", ncols=80, disable=disable_pbar_epoch
        )
        for i_epoch in range(num_epochs):
            pbar = tqdm.tqdm(
                total=num_batches_train,
                desc=f"Epoch {i_epoch + 1}|{num_epochs}",
                ncols=80,
