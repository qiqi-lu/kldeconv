...
y_bd = io.imread(os.path.join(path_result_sample, "deconvblind", "deconv.tif")).astype(
    np.float32
)
y_trad = io.imread(
    os.path.join(path_result_sample, "traditional", "deconv.tif")
).astype(np.float32)
...
for i in range(N_meth):
    ...
    axes[1, i].imshow(
        img[pos[0] : pos[0] + size[0], pos[1] : pos[1] + size[1]], **dict_img
    )
...
# profile line
fig, axes = plt.subplots(
    nrows=1, ncols=1, dpi=300, figsize=(6, 3), constrained_layout=True
)
