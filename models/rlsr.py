...
from models import rln_2d, rrdbnet, swinir, srcnn, unet, ddn, srgan
...
class Upsampler_UC(nn.Module):
    ...
    def forward(self, x):
        if self.scale_factor == 4:
            fea = self.act(
                self.conv1(
                    nn.functional.interpolate(input=x, scale_factor=2, mode="nearest")
                )
            )
            fea = self.act(
                self.conv2(
                    nn.functional.interpolate(input=fea, scale_factor=2, mode="nearest")
                )
            )
        ...
        if self.scale_factor == 3:
            fea = self.act(
                self.conv1(
                    nn.functional.interpolate(input=x, scale_factor=3, mode="nearest")
                )
            )
...
class Upsampler_PS(nn.Module):
    ...
    def __init__(
        self, scale=4, n_features=8, kernel_size=3, bn=False, act=False, bias=True
    ):
        super().__init__()
        modules = []
        if (scale & (scale - 1)) == 0:  # 2, 4, 8... 2^n
            for _ in range(int(math.log(scale, 2))):
                ...
                if bn == True:
                    modules.append(nn.BatchNorm2d(num_features=n_features))
                if act == "relu":
                    modules.append(nn.ReLU(inplace=True))
                if act == "prelu":
                    modules.append(nn.PReLU(num_parameters=n_features))
        elif scale == 3:
            modules.append(
                nn.Conv2d(
                    in_channels=n_features,
...
class Tail(nn.Module):
    ...
    def __init__(self, in_channels=3, n_features=8, kernel_size=3, bias=True):
        ...
        self.conv2 = nn.Conv2d(
            in_channels=n_features + in_channels,
            out_channels=n_features,
            kernel_size=kernel_size,
            stride=1,
            padding=(kernel_size // 2),
...
class image_style_transfer(nn.Module):
    ...
    def __init__(self, in_channels=3, kernel_size=3):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(
                16, 8, kernel_size=kernel_size, stride=1, padding=kernel_size // 2
            ),
            nn.ReLU(inplace=True),
            nn.Conv2d(
                8,
...
class Initializer(nn.Module):
    ...
    def __init__(self, in_channels, scale, kernel_size=3):
        super().__init__()
        self.scale = scale
        self.transfer = image_style_transfer(
            in_channels=in_channels, kernel_size=kernel_size
        )

    def forward(self, x):
        xsr = nn.functional.interpolate(
            input=x, scale_factor=self.scale, mode="bilinear"
        )
...
class ForwardProject(nn.Module):
    ...
    def __init__(
        ...
        if self.bn == True:
            self.norm = nn.BatchNorm2d(num_features=n_features)
        self.conv = nn.Conv2d(
            in_channels=n_features,
            out_channels=in_channels * fpm,
            kernel_size=kernel_size,
            stride=1,
            padding=kernel_size // 2,
            bias=bias,
    ...
    def forward(self, x):
        ...
        if self.bn == True:
            fea = nn.ReLU(inplace=True)(self.norm(fea))
...
class BackwardProject(nn.Module):
    ...
    def __init__(
        self, backbone, upsample, only_net=False, scale_factor=4, RL_version="RL"
    ):
        super().__init__()
...
class RLSR(nn.Module):
    ...
    def __init__(
        ...
        initializer=None,
        bn=True,
        upsample_mode="conv_up",
        bp_ks=3,
        use_prior=False,
        prior_type="learned",
        lambda_prior=1.0,
        train_only_prior=False,
        prior_inchannels=1,
        RL_version="RL",
        pixel_binning_mode="ave",
        constraint_01=True,
        ...
        if forw_proj == None:  # no pre-trained forward projection network
            if backbone_type == "rrdb":
                backbone_fp = rrdbnet.backbone(
                    in_channels=in_channels,
                    n_features=self.n_features[0],
                    n_blocks=n_blocks[0],
                    growth_channels=self.n_features[0] // 2,
                    bias=bias,
                )
            if backbone_type == "srcnn":
                backbone_fp = srcnn.backbone(
                    num_channels=in_channels, n_features=self.n_features[0]
                )
        ...
        upsample.append(
            nn.Conv2d(
                ...
                out_channels=in_channels * self.bpm,
                kernel_size=bp_ks,
                stride=1,
                padding=bp_ks // 2,
                bias=bias,
        ...
        if self.cat_x == True:
            in_chan = in_chan + in_channels
        if backbone_type == "rrdb":
            backbone_bp = rrdbnet.backbone(
                in_channels=in_chan,
                n_features=self.n_features[1],
                n_blocks=n_blocks[1],
                growth_channels=self.n_features[1] // 2,
                bias=bias,
                use_bn=False,
            )
        if backbone_type == "srcnn":
            backbone_bp = srcnn.backbone(
                num_channels=in_chan, n_features=self.n_features[1]
            )

        self.BP = BackwardProject(
            backbone=backbone_bp,
            upsample=upsample,
            only_net=self.only_net,
        ...
        if self.use_prior == True:
            if self.prior_type == "learned":
                in_chan = (
                    in_channels * (self.prior_inchannels - 1) + in_channels * self.bpm
                )
                backbone_pri = unet.backbone(
                    in_channels=in_chan, bilinear=False, use_bn=prior_bn
                )
                self.Prior = Prior(
                    backbone=backbone_pri,
                    n_features=8,
                    out_channels=in_channels,
                    kernel_size=3,
                )
    ...
    def forward(self, x):
        ...
        if self.only_net == False:
            ...
            if self.init_mode in ["bicubic", "bilinear", "nearest"]:
                xsr = nn.functional.interpolate(
                    input=x, scale_factor=self.scale, mode=self.init_mode
                )

            if self.init_mode == "constant":
                xsr = nn.functional.interpolate(
                    input=x, scale_factor=self.scale, mode="nearest"
                )
                xsr = torch.ones_like(xsr) * 0.5

            if self.init_mode == "ave_bicubic":
                x_ave = torch.nn.AvgPool2d(kernel_size=3, stride=1, padding=3 // 2)(
                    x
                )  # suppress noise
                xsr = nn.functional.interpolate(
                    input=x_ave, scale_factor=self.scale, mode="bicubic"
                )
            ...
            if self.output_mode in ["each-iter-train", "each-iter-train-prior"]:
                if self.init_mode == "net":
                    out.append(xsr)
            if self.output_mode in ["each-iter-test", "inters"]:
                out.append(xsr)
            ...
            for i_iter in range(self.n_iter):
                ...
                if self.RL_version == "RL":
                    ...
                    if self.cat_x == True:
                        ...
                        if self.upsample_mode == "up_conv":
                            dv_up = nn.functional.interpolate(
                                input=dv, scale_factor=self.scale, mode="bicubic"
                            )
                    ...
                    if self.use_prior:
                        ...
                        if self.prior_inchannels == 3:
                            x_inter = nn.functional.interpolate(
                                x, scale_factor=self.scale, mode="nearest"
                            )
                            prior_input = torch.cat(
                                (xsr0, x_inter, self.Edge(xsr0)), dim=1
                            )
                ...
                if self.output_mode == "inters":
                    if self.use_prior == True:
                        out.extend([fp, dv, bp, xsr, xsr0, one_pg])
                    if self.use_prior == False:
                        out.extend([fp, dv, bp, xsr])

                if self.output_mode == "each-iter-train":
                    out.append(xsr)
                if self.output_mode == "each-iter-train-prior":
                    out.extend([xsr0, xsr])
            ...
            if self.output_mode in [
                "each-iter-train",
                "each-iter-test",
                "each-iter-train-prior",
            ]:
                return torch.stack(out, dim=0)
            if self.output_mode == "inters":
                return out
            if self.output_mode == "last-one":
                return xsr
