...
def gauss_kernel_2d(shape=[3, 3], sigma=1.0):
    ...
    x_data, y_data = np.mgrid[
        -shape[0] // 2 + 1 : shape[0] // 2 + 1, -shape[1] // 2 + 1 : shape[1] // 2 + 1
    ]
...
def gauss_kernel_2d_multichannel(shape=[4, 3, 3, 3], stddev=[2.0, 0.5, 1.0, 1.5]):
    ...
    rad = (maxval - minval) * torch.rand(
        size=[1, shape[1], 1, 1], dtype=torch.float32
    ) + minval
    init = init * rad
    return init
...
class FP1(nn.Module):
    ...
    def __init__(self, in_channels=3, n_features=4, kernel_size=3, bias=False):
        ...
        self.bn_act1 = nn.Sequential(
            nn.BatchNorm2d(num_features=n_features), nn.Softplus()
        )
        ...
        self.bn_act3 = nn.Sequential(
            nn.BatchNorm2d(num_features=in_channels * 2), nn.Softplus()
        )
        ...
        with torch.no_grad():
            self.conv1.weight.data = gauss_kernel_2d_multichannel(
                shape=[n_features, in_channels, kernel_size, kernel_size],
                stddev=np.linspace(0.5, 2.0, n_features),
            )
            self.conv2.weight.data = gauss_kernel_2d_multichannel(
                shape=[n_features, n_features, kernel_size, kernel_size],
                stddev=np.linspace(0.5, 2.0, n_features),
            )
            self.conv3.weight.data = gauss_kernel_2d_multichannel(
                shape=[in_channels * 2, n_features * 2, kernel_size, kernel_size],
...
class FP2(nn.Module):
    ...
    def __init__(self, in_channels=3, n_features=4, kernel_size=3, bias=False):
        ...
        with torch.no_grad():
            ...
            self.conv2.weight.data = gauss_kernel_2d_multichannel(
                shape=[in_channels * 2, n_features, kernel_size, kernel_size],
                stddev=np.linspace(0.5, 2.0, in_channels * 2),
            )
...
class MUL(nn.Module):
    ...
    def __init__(self, in_channels):
        super().__init__()
        self.bn_act = nn.Sequential(
            nn.BatchNorm2d(num_features=in_channels), nn.Softplus()
        )
...
if __name__ == "__main__":
    ...
    print(kernel)
    kernels = gauss_kernel_2d_multichannel(
        shape=[4, 3, 3, 3], stddev=np.linspace(0.5, 2.0, 4)
    )
    print(kernels[0, 0])

    input_shape = (4, 1, 128, 128)
    x = torch.zeros(size=input_shape)
    model = RLN(scale=1, in_channels=1, n_features=4, kernel_size=3)
    o = model(x)
