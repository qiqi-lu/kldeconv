...
class FP1_3D(nn.Module):
    ...
    def __init__(self, in_channels=3, n_features=4, kernel_size=3, bias=False):
        ...
        self.bn_act3 = nn.Sequential(
            nn.BatchNorm3d(num_features=in_channels * 2), nn.Softplus()
        )

        self.act0 = nn.Softplus()

        # initialization of convolution filters (out_channels, in_channels, kernel_size, kernel_size, kernel_size)
        with torch.no_grad():
            self.conv1.weight.data = gauss_kernel_3d_multichannel(
                shape=[n_features, in_channels, kernel_size, kernel_size, kernel_size],
                stddev=np.linspace(0.5, 2.0, n_features),
            )
            self.conv2.weight.data = gauss_kernel_3d_multichannel(
                shape=[n_features, n_features, kernel_size, kernel_size, kernel_size],
                stddev=np.linspace(0.5, 2.0, n_features),
            )
    ...
    def forward(self, x):
        ...
        cat_12 = torch.cat([out_1, out_2], dim=-4)  # concat along channel dimension
        out_3 = self.bn_act3(self.conv3(cat_12))
        out = out_3 + self.act0(x.repeat(1, 2, 1, 1, 1))
        return out
...
class FP2_3D(nn.Module):
    ...
    def forward(self, x):
        out_1 = self.bn_act1(self.conv1(x))
        out_2 = self.bn_act2(self.conv2(out_1))
        out = out_2 + self.act0(x.repeat(1, 2, 1, 1, 1))
        return out
...
if __name__ == "__main__":
    ...
    model = RLN_3D(scale=1, in_channels=1, n_features=4, kernel_size=3)
    o = model(x)
