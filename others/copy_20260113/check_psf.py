...
data_input_path = os.path.join(
    dataset_path, f"input_noise_{noise_level}_crop", f"{id_data}.tif"
)
...
data_blur_poi_gaus = data_blur_poi_norm + np.random.normal(
    loc=0, scale=sigma / max_signal, size=data_blur_poi_norm.shape
)
...
axes[0, 0].imshow(data_gt[Sx // 2], cmap="gray", vmin=0, vmax=vmax_gt), axes[
    0, 0
].set_title("GT (max={:.2f})".format(data_gt.max()))
axes[0, 1].imshow(data_input[Sx // 2], cmap="gray", vmin=0, vmax=vmax_gt), axes[
    0, 1
].set_title("Blured (max={:.2f})".format(data_input.max()))
axes[0, 2].imshow(data_blur[Sx // 2], cmap="gray", vmin=0, vmax=vmax_gt), axes[
    0, 2
].set_title("Blured_c (max={:.2f})".format(data_blur.max()))
axes[1, 2].imshow(
    np.abs(data_input - data_blur)[Sx // 2], cmap="gray", vmin=0, vmax=vmax_gt * 0.1
)
axes[1, 2].set_title("Diff (max={:.2f})".format(np.abs((data_input - data_blur)).max()))
print(np.max(np.abs(data_input - data_blur)))

axes[1, 0].imshow(
    PSF_align[Sx // 2, Sy // 2 - 16 : Sy // 2 + 16, Sz // 2 - 16 : Sz // 2 + 16],
    cmap="gray",
    vmin=0,
    vmax=PSF.max(),
), axes[1, 0].set_title("PSF (xy)")
axes[1, 1].imshow(
    PSF_align[Sx // 2 - 16 : Sx // 2 + 16, Sy // 2, Sz // 2 - 16 : Sz // 2 + 16],
    cmap="gray",
    vmin=0,
    vmax=PSF.max(),
), axes[1, 1].set_title("PSF (xz)")
