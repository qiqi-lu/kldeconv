...
for id_channel in range(len(path_fig_data)):
    path_fig_sample = os.path.join(path_fig_data[id_channel], f"sample_{id_sample}")
    ...
    imgs.append(
        io.imread(
            os.path.join(path_dataset[id_channel], methods[0], raw_txt[id_sample])
        )
    )
    # conventional methods
    for meth in methods[1:-2]:
        imgs.append(io.imread(os.path.join(path_fig_sample, meth, "deconv.tif")))
    # KLD-ss
    imgs.append(
        io.imread(os.path.join(path_fig_sample, methods[-2], "y_pred_all.tif"))[-1]
    )
    # KLD
    imgs.append(io.imread(os.path.join(path_fig_sample, methods[-1], "y_pred_all.tif")))
...
print(
    "Num of channel: {}, num of methods: {}, image shape: {}".format(
        Nc, Nmeth, (Nz, Ny, Nx)
    )
)
...
fig, axes = plt.subplots(
    nrows=nr, ncols=nc, dpi=600, figsize=(3 * nc, 3 * nr), constrained_layout=True
)
...
for imeth in range(Nmeth):
    ...
    # xy plane
    # --------------------------------------------------------------------------
    xy_plane = np.max(img[center_xy - range_xy : center_xy + range_xy], axis=0)
    ...
    if imeth == 0:
        vmin = [0, 0]
        vmax = [
            np.percentile(xy_plane[..., 0], 99.99),
            np.percentile(xy_plane[..., 1], 99.9),
        ]
    else:
        vmin = [0, 0]
        vmax = [
            np.percentile(xy_plane[..., 0], 99.5),
    ...
    axes[0, 0].plot(
        (line_start[0], line_end[0]), (line_start[1], line_end[1]), **dict_line
    )
    ...
    patch = patches.Rectangle(
        xy=pos, width=size[1], height=size[0], fill=False, edgecolor="white"
    )
    axes[0, 0].add_patch(patch)
    axes[2, imeth].imshow(
        xy_plane[pos[1] : pos[1] + size[1], pos[0] : pos[0] + size[0], id_channel],
        vmax=vmax[id_channel],
        vmin=vmin[id_channel],
        cmap=cmaps[id_channel],
    )

plt.savefig(
    os.path.join(
        path_specimen, f"image_restored_{methods[imeth]}_compare_{id_sample}.png"
    )
)
plt.rcParams["svg.fonttype"] = "none"
plt.savefig(
    os.path.join(
        path_specimen, f"image_restored_{methods[imeth]}_compare_{id_sample}.svg"
    )
)
...
for imeth in range(Nmeth):
    ...
    axes[imeth].plot(profile, label=methods[imeth], color="#2A629A", **dict_profile)

io.imsave(
    fname=os.path.join(path_specimen, "xy.tif"), arr=xy_plane, check_contrast=False
)
