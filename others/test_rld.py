...
if data_set_name == "tinymicro_synth":
    ...
    lr_root_path = os.path.join(
        "data", "TinyMicro", "data_synth", "test", "sf_4_k_2.0_gaussian_0.03_ave"
    )
...
if data_set_name == "lung3_synth":
    ...
    hr_root_path = os.path.join(
        root_path, "Lung3", "manifest-41uMmeOh151290643884877939"
    )
    lr_root_path = os.path.join(
        root_path,
        "Lung3",
        ...
        "data_synth",
        "test",
        "sf_4_k_2.0_gaussian_0.03_ave",
    )

    hr_txt_file_path = os.path.join(
        root_path, "Lung3", "manifest-41uMmeOh151290643884877939", "test_txt", "hr.txt"
    )
    lr_txt_file_path = os.path.join(
        root_path, "Lung3", "manifest-41uMmeOh151290643884877939", "test_txt", "lr.txt"
    )
...
if data_set_name in [
    ...
    x, img_gt = torch.unsqueeze(ds["lr"], 0).to(device), torch.unsqueeze(
        ds["hr"], 0
    ).to(device)
    if in_channels == 3:
        img_gt = img_gt.numpy().transpose((0, 2, 3, 1))[0]
    if in_channels == 1:
        img_gt = img_gt.numpy().transpose((0, 2, 3, 1))[0, ..., 0]
...
img_init = ip.interpolation(img_down_n, scale_factor=4, mode="bicubic").to(
    torch.float32
)
img_blur, img_down, img_down_n, img_init = (
    ip.to_image(img_blur),
    ip.to_image(img_down),
    ip.to_image(img_down_n),
    ip.to_image(img_init),
)
...
fig, axes = plt.subplots(
    nrows=3, ncols=4, dpi=600, figsize=(2.4 * 4, 2.4 * 3), constrained_layout=True
)
