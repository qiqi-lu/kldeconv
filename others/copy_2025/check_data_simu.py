...
axes[0, 0].imshow(data_gt[Sx // 2], cmap="gray", vmin=0, vmax=vmax_gt), axes[
    0, 0
].set_title("GT (sum={:.2f})".format(data_gt.max()))
axes[0, 1].imshow(data_input[Sx // 2], cmap="gray", vmin=0, vmax=vmax_gt), axes[
    0, 1
].set_title("RAW (sum={:.2f})".format(data_input.max()))
axes[0, 2].plot(data_gt[Sx // 2, 100, 50:500], "red")
axes[0, 2].plot(data_input[Sx // 2, 100, 50:500], "green")

axes[1, 0].imshow(data_gt[Sx // 2 + 1], cmap="gray", vmin=0, vmax=vmax_gt), axes[
    1, 0
].set_title("GT (sum={:.2f})".format(data_gt.max()))
axes[1, 1].imshow(data_input[Sx // 2 + 1], cmap="gray", vmin=0, vmax=vmax_gt), axes[
    1, 1
].set_title("RAW (sum={:.2f})".format(data_input.max()))
