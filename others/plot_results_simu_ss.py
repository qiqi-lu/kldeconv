...
nr, nc = 2, 3
fig, axes = plt.subplots(
    nrows=nr, ncols=nc, dpi=300, figsize=(3.6 * nc, 3.6 * nr), constrained_layout=True
)
...
axes[0, 0].text(
    ...
    pos_text_y,
    "RAW (xy) ({:>.2f}, {:>.4f})".format(
        cal_psnr(data_input, data_gt), cal_ssim(data_input, data_gt)
    ),
    **dict_text,
)
...
#     'WB (2)']
methods_name = ["Traditional (2)", "Gaussian (2)", "Butterworth (2)", "WB (2)"]
colors = [
    "#B3BE9D",
    "#FDE767",
    "#F3B95F",
...
fig, axes = plt.subplots(
    nrows=1, ncols=2, dpi=300, figsize=(6, 3), constrained_layout=True
)
...
for i, ax in enumerate(axes.ravel()):
    if i == 0:
        ...
        profile = profile_line(data_input[id_slice], line_start, line_end, linewidth=1)
    ...
    if i == 1:
        ...
        profile = profile_line(
            data_input[:, Ny // 2, :], line_start, line_end, linewidth=1
        )
        ax.plot(profile, label="RAW", color="#2A629A", **dict_profile)
