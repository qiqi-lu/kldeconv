...
training_data = data.CytoDataset_synth(
    txt_file=training_data_txt,
    dir_hr=dir_hr,
    dir_synth=dir_synth,
...
model_path = os.path.join(
    checkpoint_dir,
    "{}_bs_{}_lr_{}_{}".format(model_name, batch_size, learning_rate, model_suffix),
)
...
scheduler = optim.lr_scheduler.StepLR(
    optimizer=optimizer, step_size=scheduler_cus["every"], gamma=scheduler_cus["rate"]
)
...
for t in range(start_epoch + 1, epochs):
    ...
    pbar = tqdm.tqdm(
        total=num_batches, desc="TRAINING", leave=True, ncols=150
    )  # processing bar
    ...
    for batch, sample in enumerate(train_dataloader):
            ...
            if i % scheduler_cus["every"] == 0:
                for g in optimizer.param_groups:
                    g["lr"] = scheduler_cus["lr"] * (
                        scheduler_cus["rate"] ** (i // scheduler_cus["every"])
                    )
        ...
        if data_transform_back is not None:
            pred, y, x = (
                ...
                data_transform_back(y),
                data_transform_back(x),
            )
        ...
        if i % every_batch == 0:
            ave_ssim, ave_psnr = eva.measure(
                pred=train.rescale(out), y=train.rescale(y), x=train.rescale(x)
            )
            ...
            if writer != None:
                ...
                writer.add_images(
                    ...
                    torch.nn.functional.interpolate(
                        x[:, permute], scale_factor=4, mode="nearest"
                    ),
                    i,
                )
        ...
        pbar.set_postfix_str(
            s="loss: {:>7f}, PSNR: {:>.4f}, SSIM: {:>.4f}, lr: {:>.7f}".format(
                loss.item(), ave_psnr, ave_ssim, optimizer.param_groups[0]["lr"]
            )
        )
        pbar.update(1)
        ...
        if (i + 1) % 10000 == 0:
            ...
            torch.save(
                model_dict, os.path.join(model_path, "epoch_{}_{}.pt".format(t, i + 1))
            )
            print("> Save model ...")
